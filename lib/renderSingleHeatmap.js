...
module.exports = renderSingleHeatmap = function(opts, data){
	...
	var labelCounts = data.colLabelCounts();
	...
	for(var i in values){
	 ...
	 'translate(0,' + heightOffset + ')');
		...
		.attr('y', function (d) { return 0.3; })
		.attr('height', function (d) { 
			return localValueHeight - 0.5; 
		})
		.attr('width', function (d) { return opts.valueWidth - 0.6; })
	...
	chrG.attr('transform', 'translate('+ (1 + totalLines)  * opts.valueWidth +',0)')
	chrG.append('line')
	.attr('x1', 0)
	.attr('y1', 0)
	.attr('x2', 0)
	.attr('y2', midCentromereOffset - (0.035 * opts.height))
	...
	chrG.append('line')
	.attr('x1', 0)
	.attr('y1', midCentromereOffset + (0.05*localValueHeight))
	.attr('x2', 0)
	...
	.enter()
	.append('text');
	txts.style('fill', function(d){
		return labelCounts[d] > 1 ? 'red':'black';
	})
